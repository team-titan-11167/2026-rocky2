...
public class DriveCommands {
  ...
  /** Drives from normalized arcade inputs using the configured closed-loop maximum speed. */
  public static Command arcadeDriveNormalized(
      Drive drive, DoubleSupplier xSupplier, DoubleSupplier zSupplier) {
    return Commands.run(
        () -> runArcadeNormalized(drive, xSupplier.getAsDouble(), zSupplier.getAsDouble()), drive);
  }

  /** Applies WPILib's squared-input arcade mixing to normalized inputs. */
  public static void runArcadeNormalized(Drive drive, double xSpeed, double zRotation) {
    var speeds = DifferentialDrive.arcadeDriveIK(xSpeed, zRotation, true);
    drive.runClosedLoop(speeds.left * maxSpeedMetersPerSec, speeds.right * maxSpeedMetersPerSec);
  }
