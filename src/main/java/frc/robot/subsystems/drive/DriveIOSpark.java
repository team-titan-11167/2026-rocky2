...
public class DriveIOSpark implements DriveIO {
  ...
  public DriveIOSpark() {
    ...
    config.inverted(false).follow(leftLeader);
    tryUntilOk(
        leftFollower,
        5,
        () ->
            leftFollower.configure(
                config, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters));
    config.inverted(false).follow(rightLeader);
    tryUntilOk(
        rightFollower,
        5,
        () ->
  ...
  public void updateInputs(DriveIOInputs inputs) {
    ...
    ifOk(
        rightLeader,
        new DoubleSupplier[] {rightLeader::getOutputCurrent, rightFollower::getOutputCurrent},
